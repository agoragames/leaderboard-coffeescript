...
(function() {
  ...
  Leaderboard = (function() {
    ...
    Leaderboard.DEFAULT_REDIS_OPTIONS = {};
    ...
    DEFAULT_REDIS_OPTIONS = {
      ...
      'options': Leaderboard.DEFAULT_REDIS_OPTIONS
    };
    ...
    function Leaderboard(leaderboardName, options, redisOptions) {
      ...
      if (this.redisConnection == null) {
        this.redisConnection = redis.createClient(redisOptions['port'], redisOptions['host'], redisOptions['options']);
      }
      if (redisOptions.hasOwnProperty('auth_pass')) {
        this.redisConnection.auth(redisOptions.options.auth_pass);
      }
    }
    ...
    Leaderboard.prototype.rankMemberAcross = function(leaderboardNames, member, score, memberData, callback) {
      var i, leaderboardName, len1, transaction;
      if (memberData == null) {
        memberData = null;
      }
      transaction = this.redisConnection.multi();
      for (i = 0, len1 = leaderboardNames.length; i < len1; i++) {
    ...
    Leaderboard.prototype.rankMembersIn = function(leaderboardName, membersAndScores, callback) {
      var i, index, ref, slice, transaction;
      transaction = this.redisConnection.multi();
      for (index = i = 0, ref = membersAndScores.length; i < ref; index = i += 2) {
    ...
    Leaderboard.prototype.rankedInListIn = function(leaderboardName, members, options, callback) {
      var i, len1, member, ranksForMembers, transaction;
      if (options == null) {
        options = {};
      }
      ...
      if (!options['membersOnly']) {
        for (i = 0, len1 = members.length; i < len1; i++) {
          member = members[i];
          if (this.reverse) {
            transaction.zrank(leaderboardName, member);
          } else {
      ...
      return transaction.exec((function(_this) {
        return function(err, replies) {
          var index, j, len2, results;
          results = [];
          for (index = j = 0, len2 = members.length; j < len2; index = ++j) {
            member = members[index];
            results.push((function(member) {
              var data;
              data = {};
